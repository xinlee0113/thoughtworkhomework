...
//            tvId.setText(item.id+"");
        ...
        private void bindTweetComment(TweetEntity item) {
            ...
            if (tweetCommentAdapter == null) {
                tweetCommentAdapter = new BaseQuickAdapter<TweetEntity.Comment, BaseViewHolder>(R.layout.item_tweet_comment, item.comments) {
                    @Override
                    protected void convert(@NotNull BaseViewHolder baseViewHolder, TweetEntity.Comment comment) {
                        ...
                        Glide.with(sendAvatarImgView).load(comment.sender.avatar).into(sendAvatarImgView);
                    }
                };
        ...
        private void bindTweetImage(TweetEntity item) {
            ...
            if (tweetImgAdapter == null) {
                tweetImgAdapter = new BaseQuickAdapter<TweetEntity.Image, BaseViewHolder>(R.layout.item_tweet_img, item.images) {
                    @Override
                    protected void convert(@NotNull BaseViewHolder baseViewHolder, TweetEntity.Image img) {
                        ImageView imgItemView = baseViewHolder.findView(R.id.img_tweet);
                        Glide.with(imgItemView).load(img.url).error(R.drawable.img_t).into(imgItemView);
                    }
                };
